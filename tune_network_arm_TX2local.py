...
parser.add_argument(
      '-m', '--model', required=True, help=('Model defined in realy. Examples: {resnet-18, mobilenet, squeezenet_v1.1, inception_v3, vgg-19}, etc'))
parser.add_argument(
      '--layout', default="NCHW", choices=("NCHW", "NHWC"), help='Layout for etworks from relay. {NCHW, NHWC}')
parser.add_argument(
       '--target', required=False, default="llvm -mtriple=aarch64-linux-gnu -mattr=+neon", help='Target')
parser.add_argument(
      '--batch', default=1, help='Batch size')
parser.add_argument(
       '--ntrials', required=False, default=20000, help='number of measurement trials')
parser.add_argument(
       '--logfile', required=False, default="", help='Output tuning log file')
...
if log_file == "":
    log_file = "%s-%s-B%d-%s-tx2.json" % (network, layout, batch_size, target.kind.name)
...
def tune_and_evaluate():
    ...
    tune_option = auto_scheduler.TuningOptions(
        num_measure_trials=int(args.ntrials),  # change this to 20000 to achieve the best performance
