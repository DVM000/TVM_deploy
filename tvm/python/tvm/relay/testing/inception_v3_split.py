...
def get_net(N, batch_size, num_classes, image_shape, dtype):
    ...
    if N == 5:
        net1 = conv_3
        conv_3 = relay.var("data", shape=(batch_size, 80,73,73), dtype=dtype)
    ...
    # stage 4
    #data = relay.var("data", shape=(batch_size, 768,17,17), dtype=dtype)
    ...
    if N == 88:
        net1 = in4d
        in4d = relay.var("data", shape=(batch_size, 768,17,17), dtype=dtype)
    
    in4e = Inception7D(in4d, 192, 320, 192, 192, 192, 192, "max", "mixed_8")
    # stage 5
    #data = relay.var("data", shape=(batch_size, 1280,8,8), dtype=dtype)
    in5a = Inception7E(in4e, 320, 384, 384, 384, 448, 384, 384, 384, "avg", 192, "mixed_9")
    if N == 107:
        net1 = in5a
        in5a = relay.var("data", shape=(batch_size, 2048,8,8), dtype=dtype)
    ...
    if N == 119:
        net1 = pool
        pool = relay.var("data", shape=(batch_size, 2048,1,1), dtype=dtype)
    ...
    return [relay.Function(relay.analysis.free_vars(net1), net1),  relay.Function(relay.analysis.free_vars(inception_v3), inception_v3)]

def get_workload(N=88, npartition=0, batch_size=1, num_classes=1000, image_shape=(3, 299, 299), dtype="float32"):
