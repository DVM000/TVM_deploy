...
def get_feature_noPool(internal_layer, layers, filters, batch_norm=False):
    """Get VGG feature body as stacks of convolutions."""
    for i, num in enumerate(layers):
        for j in range(num):
            internal_layer = wrapper.conv2d(
            ...
            internal_layer = relay.nn.relu(data=internal_layer)
        if i<len(layers)-1: internal_layer = relay.nn.max_pool2d(data=internal_layer, pool_size=(2, 2), strides=(2, 2))
    return internal_layer
...
def get_net(N, batch_size, image_shape, num_classes, dtype, num_layers=11, batch_norm=False):
    """
    Parameters
    ----------
    batch_size : int
    ...
    if N==20:
        # Partition point before last pool
        net1 = get_feature_noPool(data, layers, filters, batch_norm)
        data1 = relay.var("data", shape=(data_shape[0],512,14,14), dtype=dtype)
        data1 = relay.nn.max_pool2d(data=data1, pool_size=(2, 2), strides=(2, 2))

    classifier = get_classifier(data1, num_classes)
    symbol = relay.nn.softmax(data=classifier)
    args = relay.analysis.free_vars(symbol)
    #return relay.Function(args, symbol)
    return [relay.Function(relay.analysis.free_vars(net1), net1), relay.Function(args, symbol)]
...
def get_workload(
    ...
    num_layers=19
    if  N != 20:
        print('vgg-19 partition with {} layers not implemented'.format(N))
        import sys; sys.exit(1)
    net = get_net(N,batch_size, image_shape, num_classes, dtype, num_layers, batch_norm)
